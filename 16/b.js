...
const node2str = (_n) => {
  return _n[0] + "_" + _n[1] + "_" + _n[2] + "_" + _n[3];
};
...
for (let i = 0; i < _map.length; i++) {
  ...
  let node = [i, -1, 0, 1];
  calc_road(node);
  let _count = calc_count(_ways);
  cnts.push([node2str(node), _count]);

  _ways.length = 0;
  node = [i, _map[0].length, 0, -1];
  calc_road(node);
  _count = calc_count(_ways);
  cnts.push([node2str(node), _count]);
}
...
for (let i = 0; i < _map[0].length; i++) {
  ...
  _ways.length = 0;
  node = [_map.length, i, -1, 0];
  calc_road(node);
  _count = calc_count(_ways);
  cnts.push([node2str(node), _count]);
}

cnts.sort((a, b) => b[1] - a[1]);

console.log("A", cnts[0]);
